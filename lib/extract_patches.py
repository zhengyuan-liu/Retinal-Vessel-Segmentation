...
def extract_random_rotate(full_imgs,full_masks, patch_h,patch_w, N_patches, inside=True):
    ...
    for i in range(full_imgs.shape[0]):  # loop over the full images
        ...
        while k < patch_per_img:
            ...
            patch90 = np.rot90(patch, k=1, axes=(1, 2))
            patch_mask90 = np.rot90(patch_mask, k=1, axes=(1, 2))
            patches[iter_tot + 1] = patch90
            patches_masks[iter_tot + 1] = patch_mask90
            patch180 = np.rot90(patch, k=2, axes=(1, 2))
            patch_mask180 = np.rot90(patch_mask, k=2, axes=(1, 2))
            patches[iter_tot + 2] = patch180
            patches_masks[iter_tot + 2] = patch_mask180
            patch270 = np.rot90(patch, k=3, axes=(1, 2))
            patch_mask270 = np.rot90(patch_mask, k=3, axes=(1, 2))
            patches[iter_tot + 3] = patch270
            patches_masks[iter_tot + 3] = patch_mask270
            iter_tot += 4   # total
            k += 4  # per full_img
